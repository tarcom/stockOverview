...
public class MomentumInvestmentStrategy {
    ...
    public static void doRun(int daysHist) throws Exception {
        ...
        int count = 0;
        for (StockWrapper stock : momentumMap.values()) {
            if (count++ > momentumMap.size() - 10) {
                googleFinanceCopyPasteString += "CPH%3A" + stock.stock.getSymbol().replace(".CO", "") + "%2C";
            }
        }
        ...
        //https://www.google.com/finance?q=CPH%3AGEN%2CCPH%3AGJ%2CCPH%3AHOEJ-B&ei=6vTvVdCDBNXcsAGe95GwBA
        googleFinanceCopyPasteString = "https://www.google.com/finance?q=";
        for (Double score : momentumMap.keySet()) {
            if (score > 20) {
                StockWrapper stock = momentumMap.get(score);
                googleFinanceCopyPasteString += "CPH%3A" + stock.stock.getSymbol().replace(".CO", "") + "%2C";
            }
        }
        googleFinanceCopyPasteString = googleFinanceCopyPasteString.substring(0, googleFinanceCopyPasteString.length() - 3);
